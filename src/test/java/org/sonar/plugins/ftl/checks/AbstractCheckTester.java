 ...
 */
package org.sonar.plugins.ftl.checks;
...
import org.sonar.plugins.ftl.TestProfileWithAllRules;
...
public abstract class AbstractCheckTester extends org.sonar.plugins.ftl.AbstractFtlPluginTester {
  ...
  private Rule getRule(String ruleKey, Class<? extends AbstractPageCheck> checkClass) {
    ...
    List<Rule> rules = parser.parse("Ftl", Arrays.asList(new Class[] {checkClass}));
    for (Rule rule : rules) {
  ...
  public FtlSourceCode parseAndCheck(Reader reader, Class<? extends AbstractPageCheck> checkClass, String... params) {

    return parseAndCheck(reader, null, null, checkClass, params);
  }

  public FtlSourceCode parseAndCheck(Reader reader, java.io.File file, String code, Class<? extends AbstractPageCheck> checkClass,
      String... params) {
    ...
    FtlSourceCode ftlSourceCode = new FtlSourceCode(new File("test"));
  ...
  private static final class FtlRuleFinder implements RuleFinder {

    private final Rule rule;

    public FtlRuleFinder(Rule rule) {
      this.rule = rule;
    }
  ...
  private void configureDefaultParams(AbstractPageCheck check, Rule rule) {
    FtlRuleFinder ruleFinder = new FtlRuleFinder(rule);
